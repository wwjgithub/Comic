...
//---- MOVE --------------------------------------------------------------------------------------------
                ...
                /**
                 * Moves the selected com.k7k7.cartoonMaker.items by a certain number of pixels on the x axis and y axis
                 *
...
//---- SCALE -------------------------------------------------------------------------------------------
                 ...
                 *
                 * @param $sx Multiplier for scaling along the selection box's x-axis (which may or may not be the same as the selected item's y-axis, depending on whether or not multiple com.k7k7.cartoonMaker.items are selected and if any are rotated)
                 * @param $sy Multiplier for scaling along the selection box's y-axis (which may or may not be the same as the selected item's y-axis, depending on whether or not multiple com.k7k7.cartoonMaker.items are selected and if any are rotated)
                 * @param $angle Angle at which the item should be scaled (in Radians)
                 * @param $checkBounds If false, bounds will be ignored
                 * @param $dispatchEvent If false, no SCALE events will be dispatched
                 *
                 */
                ...
                public function scaleRotated($sx:Number, $sy:Number, $angle:Number, $skew:Number, $checkBounds:Boolean = true, $dispatchEvent:Boolean = true):void {
                        if (!_lockScale) {
                                ...
                                if (_scaleMode == "scaleNormal") {
                                        ...
                                        var p:Point = _targetObject.parent.globalToLocal(_proxy.localToGlobal(_offset)); //had to use _targetObject.parent instead of _proxy.parent because of another bug in Flex that prevented _proxy com.k7k7.cartoonMaker.items from accurately reporting their parent for a few frames after being added to the display list! Since they both have the same parent, this shouldn't matter though.
                                        //if (_scaleMode == "scaleWidthAndHeight") {
										(_targetObject as Object).setSize(w, h);
                                        //} else {
                                ...
                                if (_bounds != null) {
                                        if (!_bounds.containsRect(_target.getBounds(_target.parent))) {
                                                if ($safe.sy == 1) {
                                                        ...
                                                        /* potential future replacement technique - needs refinement for when there are skewed com.k7k7.cartoonMaker.items near the edge
