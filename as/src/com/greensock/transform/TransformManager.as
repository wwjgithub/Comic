...
package com.greensock.transform {
	 ...
	 * <ul>
	 *	   <li> Select multiple citems and scale/rotate/move them all simultaneously. </li>
	...
	public class TransformManager extends EventDispatcher {
		 ...
		 * Constructor
		 *
		 * @param $vars An object specifying any properties that should be set upon instantiation,
		 */
		...
		protected function init($vars:Object):void {
			...
			_lineColor = setDefault($vars.lineColor, 0x000000); //Line color (including handles and selection around MovieClip)
		...
		public function addItem($targetObject:DisplayObject, $scaleMode:String = "scaleNormal", $hasSelectableText:Boolean = false):TransformItem {
			...
			var newItem:TransformItem = new TransformItem($targetObject, newVars);
...
//---- GENERAL -------------------------------------------------------------------------------------------------------------------------
		 ...
		 *
		 * @param $e Accepts an optional Event in case you want to use this as an events handler
		 */
...
//---- SELECTION ---------------------------------------------------------------------------------------------------------------------
		...
		public function selectItem($item:*, $addToSelection:Boolean = false):TransformItem { //You can pass in a reference to the DisplayObject or its associated TransformItem.
			...
			if (item == null) {
				...
				_multiSelectMode = $addToSelection; //otherwise when the item dispatches a SELECT events, it'll wipe out the other selected items.
				_ignoreEvents = true;
				item.selected = true;
				_ignoreEvents = false;
...
//---- STATIC FUNCTIONS -------------------------------------------------------------------------------------------------
		...
		private static function bringToFront($o:DisplayObject):void {
			if ($o.parent != null && $o.parent.numChildren>1) {
